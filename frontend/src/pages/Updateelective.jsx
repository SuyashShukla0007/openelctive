...
const UpdateElective = () => {
    ...
    const fetchElectives = async () => {
      try {
        ...
        const electivesResponse = await axios.get(
          "https://backend-mgrr.vercel.app//api/eligible-subjects",
          {
        ...
        const selectedResponse = await axios.get(
          "https://backend-mgrr.vercel.app//api/student/selected-electives",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedElectives = Object.values(selectedValues).filter(
      (value) => value !== null
    );
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        "https://backend-mgrr.vercel.app//api/student/update-electives",
        { selectedElectives },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      <header className="bg-red-500 text-white py-4 shadow-md flex items-center justify-between px-6">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-xl font-semibold">
            NIT Kurukshetra - Elective Selection
          </h1>
        </div>
      </header>
              ...
              {options.map((option, index) => (
                <div key={`field${index}`} className="form-group mb-4">
                  ...
                  >
                    ...
                    {getFilteredOptions(`field${index}`).map(
                      (filteredOption) => (
                        <option
                          ...
                          style={{
                            ...
                            fontWeight: filteredOption.isSelected
                              ? "normal"
                              : "bold",
                          }}
                          disabled={filteredOption.isSelected}
                        >
                          {filteredOption.name} ({filteredOption.code}) -{" "}
                          {filteredOption.branch}
                        </option>
                      )
                    )}
                  </select>
